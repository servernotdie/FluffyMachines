...
public class Barrel extends NonHopperableBlock implements DoubleHologramOwner {
    ...
    private final ItemStack HOLOGRAM_OFF_ITEM = new CustomItemStack(Material.QUARTZ_SLAB, "&3Hiển thị hologram &c(Tắt)");
    private final ItemStack HOLOGRAM_ON_ITEM = new CustomItemStack(Material.QUARTZ_SLAB, "&3Hiển thị hologram &a(Bật)");
    private final ItemStack TRASH_ON_ITEM = new CustomItemStack(SlimefunItems.TRASH_CAN, "&3Hủy đầu vào khi đầy &a(Bật)",
        "&7Khi bật, sẽ hủy vật phẩm không thể lưu trữ");
    private final ItemStack TRASH_OFF_ITEM = new CustomItemStack(SlimefunItems.TRASH_CAN, "&3Hủy đầu vào khi đầy &c(Tắt)",
        "&7Khi bật, sẽ hủy vật phẩm không thể lưu trữ"
    );
            ...
            public void onPlayerBreak(@Nonnull BlockBreakEvent e, @Nonnull ItemStack item, @Nonnull List<ItemStack> drops) {
                ...
                if (inv != null) {
                    ...
                    if (breakOnlyWhenEmpty.getValue() && stored != 0) {
                        Utils.send(p, "&cVui lòng lấy các vật phẩm bên trong trước khi phá hủy!");
                        e.setCancelled(true);
                        return;
                    }
                    ...
                    if (itemCount > 5) {
                        Utils.send(p, "&eVui lòng lấy các vật phẩm bên trong trước khi phá hủy!");
                        e.setCancelled(true);
                        return;
                    }
                    ...
                    if (stored > 0) {
                        ...
                        if (stored > OVERFLOW_AMOUNT) {
                            ...
                            Utils.send(p, "&eRương Fluffy có hơn " + OVERFLOW_AMOUNT + " vật phẩm! " +
                                "Chỉ rơi " + OVERFLOW_AMOUNT + " vật phẩm!");
    ...
    protected void buildMenu(BlockMenu menu, Block b) {
        ...
        if (StorageCacheUtils.getData(b.getLocation(), "stored") == null) {
            ...
            menu.replaceExistingItem(STATUS_SLOT, new CustomItemStack(
                    Material.LIME_STAINED_GLASS_PANE, "&6Đã lưu trữ: &e0" + " / " + capacity, "&70%"));
            menu.replaceExistingItem(DISPLAY_SLOT, new CustomItemStack(Material.BARRIER, "&cKhông có vật phẩm"));

            setStored(b, 0);

            if (showHologram.getValue()) {
                updateHologram(b, null, "&cKhông có vật phẩm");
            }
        ...
        menu.replaceExistingItem(INSERT_ALL_SLOT,
                new CustomItemStack(Material.LIME_STAINED_GLASS_PANE, "&bNạp nhanh",
                        "&7> Nhấp để chuyển tất cả vật phẩm trong túi", "&7vào Rương Fluffy"));
        ...
        menu.replaceExistingItem(EXTRACT_SLOT,
                new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&6Rút nhanh",
                        "&7> Chuột trái để rút vật phẩm và lấp đầy túi",
                        "&7> Chuột phải để rút 1 vật phẩm"
                ));
    ...
    public void updateMenu(Block b, BlockMenu inv, boolean force, int capacity) {
        ...
        if (inv.hasViewer() || force) {
            inv.replaceExistingItem(STATUS_SLOT, new CustomItemStack(
                Material.LIME_STAINED_GLASS_PANE, "&6Đã lưu trữ: &e" + stored + " / " + capacity,
                "&b" + storedStacks + " ngăn &8| &7" + storedPercent + "&7%"));
        }
        ...
        if (stored == 0) {
            inv.replaceExistingItem(DISPLAY_SLOT, new CustomItemStack(Material.BARRIER, "&cKhông có vật phẩm"));
            if (showHologram.getValue() && (hasHolo == null || hasHolo.equals("true"))) {
                updateHologram(b, null, "&cKhông có vật phẩm");
            }
        }
    ...
    public void extract(Player p, BlockMenu menu, Block b, ClickAction action) {
        ...
        if (action.isRightClicked()) {
            ...
            Utils.send(p, "&cRương Fluffy trống rỗng!");
            return;
        }
    ...
    public enum BarrelType {
        ...
        SMALL(17280000, "&eRương Fluffy Cấp 1", Material.BEEHIVE, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.OAK_LOG)),
        MEDIUM(34560000, "&6Rương Fluffy Cấp 2", Material.BARREL, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.SMOOTH_STONE)),
        BIG(69120000, "&bRương Fluffy Cấp 3", Material.SMOKER, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.BRICKS)),
        LARGE(138240000, "&aRương Fluffy Cấp 4", Material.LODESTONE, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.IRON_BLOCK)),
        MASSIVE(276480000, "&5Rương Fluffy Cấp 5", Material.CRYING_OBSIDIAN, SlimefunItems.REINFORCED_PLATE, new ItemStack(Material.OBSIDIAN)),
        BOTTOMLESS(1728000000, "&cRương Fluffy Cấp 6", Material.RESPAWN_ANCHOR, SlimefunItems.BLISTERING_INGOT_3, SlimefunItems.REINFORCED_PLATE);
