    ...
    public ItemHandler getItemHandler() {
        return (ItemUseHandler) e -> {
            ...
            if (sfItem == null || sfItem != FluffyItems.ADVANCED_CHARGING_BENCH.getItem()) {
                Utils.send(e.getPlayer(), "&cBạn chỉ có thể sử dụng vật phẩm này trên Bàn sạc cao cấp");
                return;
            }
            ...
            if (tier == 100) {
                Utils.send(e.getPlayer(), "&cBàn sạc cao cấp này đã đạt cấp tối đa (100)");
                return;
            }
            tier++;
            ...
            Utils.send(e.getPlayer(), "&aBàn sạc cao cấp đã được nâng cấp! &eCấp: " + tier);
        };
    }
}
